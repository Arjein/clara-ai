...
def load_saved_threads(base_path='threads'):
    ...
    os.makedirs(os.path.join(base_path, "replied"), exist_ok=True)
    ...
    replied_files = [f for f in os.listdir(os.path.join(base_path, "replied")) 
                    if f.startswith("thread_") and f.endswith(".json")]
    
    print(f"Found {len(thread_files)} active threads and {len(replied_files)} replied threads")
    ...
    for file_name in replied_files:
        file_path = os.path.join(base_path, "replied", file_name)
        try:
            with open(file_path, 'r') as file:
                thread_data = json.load(file)
                thread = MailThread.fromJson(thread_data)
    ...
    threads.sort(key=lambda t: t.last_updated if t.last_updated else datetime.datetime.min, reverse=True)
...
def get_threads_require_process(all_threads):
    ...
    for thread in all_threads:
        if thread.pre_reply_class and thread.reply_class == None and thread.replied == False:
